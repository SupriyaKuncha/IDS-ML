...
def runSVM():
    ...
    X_train1 = SelectKBest(chi2).fit_transform(X_train, y_train)
    X_test1 = SelectKBest(chi2).fit_transform(X_test,y_test)
...
def runDT():
    ...
    DT_acc = cal_accuracy(y_test, prediction_data,'DecisionTreeClassifier Accuracy, Classification Report & Confusion Matrix')
...
def runRFT():
    ...
    RFT_acc = cal_accuracy(y_test, prediction_data,'RandomForestClassifier, Classification Report & Confusion Matrix')
...
def runLR():
    ...
    LR_acc = cal_accuracy(y_test, prediction_data,'LogisticRegression Accuracy, Classification Report & Confusion Matrix')
...
def runANN():
    ...
    model = Sequential()
    model.add(Dense(30, input_dim=15, activation='relu'))  # Adjust input_dim based on the selected features
